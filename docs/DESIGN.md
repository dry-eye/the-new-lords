...
The terrain layer is drawn to a deliberately high, at-a-glance-legible standard: each biome kind is told apart by a distinct **pattern/texture**, not colour alone — forest as a tree-stipple, plains as open ground, mountains as hachured relief, desert as a fine speckle — extending the noise-ramp direction above into a legible cartographic style that reads like a hand-drawn staff map and re-tints cleanly per visual theme (see "Visual themes," above). **Forest legibility is functional, not just decorative:** a forest zone must be unmistakable, because a lumber camp can only be built in or near one (see "Terrain zones," above) — the player has to see where wood can come from.
...
What's fixed here is the *legibility standard* — biomes distinct by pattern, forest unmistakable, rivers/lakes/fields present and readable — not the specific brushwork, which is a visual-quality pass iterated in the prototype.
