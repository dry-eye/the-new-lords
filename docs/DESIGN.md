...
- **Leader count** derives from type/attributes: monarch 1, council ~9, mercenary 2, economic 1, anarchic 0 (no formal leader, headless) — **except `kind:'squad'` and `kind:'network'` orgs, which always have exactly 1 leader regardless of leadership type.** This is a deliberate, resolved override: the leader-centric squad simulation's foundational claim that a squad has exactly one intelligent leader takes priority over the general leadership-type table for squad-kind orgs specifically, and a `network`-kind org answers to that same single-handler model (see "Agent networks," below). A squad or network that wants a second leadership seat gets there only through the **Council of Equals** perk (below), never through leadership-type alone.
...
- **Viewpoint** — a single control, defaulting to **Spectator**: an omniscient view showing every edge, covert slots and agent edges included, regardless of ownership or exposure. Switching it to a specific organization applies a participant filter instead: every public edge (control, influence, alliance, investment, ownership) stays visible as always, but a hidden edge (covert slot, agent edge, secret ownership) only shows if the selected organization owns it, or if it's already been exposed against that organization — everything else hidden. Switching back to Spectator clears the filter.
...
**Org kind axis — nature vs governance.** `org.kind` is an axis orthogonal to leadership type: `kind ∈ {squad, political, business, network}` (with `religious` added later, and criminal/media as future possibilities) describes an org's **nature**, while `leadershipType` describes its **governance profile** — the two vary independently (a squad-kind org can have any leadership type, etc.). **Loyalty/opinion stays keyed on leadership type, not kind** — the two systems must not be conflated. Kind can change at runtime, mirroring how leadership type can change.
...
| **Enterprise** | click an outside-type enterprise marker (mine, lumberjack camp, oil depot), or its entry in a settlement's enterprise list (inside-type: forge, armory, factory, component factory, construction company) | Enterprise detail view | type-prefixed label (e.g. "Oil depot · Corburg"), kind, owner org (clickable), host settlement (clickable), staffed/idle status, output rate, current stock |
