...
**Three fidelity regimes**, assigned by strategic importance, applied world-wide by game-state signals and additionally by where the camera is focused (see the hot-region trigger, below):
...
**Hot-region trigger — resolved.** A region is "hot" (materialized to full individual simulation) whenever any of the following is currently true within a fixed world-space radius of it, decided by game-state signals **and by camera focus**: an active combat, a squad executing a player-issued order, a scripted/world event in progress, **a squad belonging to the player's own side simply being present, regardless of whether it currently has an active order** (the player cares what their own squads are doing even when idle or auto-controlled, so their mere presence is its own qualifying signal), or **the camera being focused closely on that region** (a player looking closely at a place is itself a signal to simulate it in full — a deliberate choice in favour of "detail where you look," accepting the determinism cost handled below). One fixed radius applies uniformly to every signal type (a tunable constant, not per-signal-type tuning) — simplicity over precision, revisit only if playtesting shows a specific signal type clearly needs a different radius. Any number of regions can be hot simultaneously — there's no cap of one — so several distant locations each holding one of the player's own squads all stay fully simulated at once. A region stays hot for a few seconds (a tunable grace period, proposed default 5–10s) after its last qualifying signal ends before demoting back to a cohort, to avoid flicker at the boundary when a fight, order, or the player's own squad briefly leaves. This keeps events happening elsewhere in the world properly simulated even when the player isn't currently looking at them. Looking closely at a region is therefore no longer merely cosmetic — the camera materializes that region into full individual simulation like any other qualifying signal; a purely-decorative far crowd or heatmap may still be layered on top where full simulation isn't warranted.

**Render-LOD and simulation-fidelity are still distinct axes, but the camera now feeds both.** Render-LOD stays camera-zoom-driven (see "Camera controls," under UI, input & camera); simulation-fidelity is driven by the game-state signals above plus camera focus. So a player-owned squad materialized by a game-state signal stays fully simulated regardless of render zoom (a distant owned squad can be fully simulated while drawn as a simple far-zoom glyph), while the camera additionally contributes its own simulation-materializing signal on top — the axes overlap through the camera rather than being fully decoupled.

**Determinism across transitions — relative to recorded inputs.** Materializing and dematerializing a region happens at a fixed point in the tick order, driven by the seeded simulation RNG together with the recorded input stream — player orders **and the camera focus/path, which is now itself a recorded input** — never by wall-clock time. Because the camera contributes a materialization signal, determinism is *relative to those recorded inputs* rather than independent of the camera: replaying from the same seed **and the same recorded inputs, including the camera track**, reproduces the same hot regions at the same ticks. Save and replay must therefore persist the camera focus alongside player orders (see "Open question: save/load consistency," below).
...
The hot-region/cohort materialize system above is an instance of a broader rule: **detail (individual sim vs. abstract aggregate) scales with proximity to player-controlled squads/avatars and with hot-region signals (combat, player focus, events, and camera focus — the camera is a recorded input, so this stays deterministic relative to recorded inputs), uniformly across every simulated subsystem** — not just pawns, and not just rendering. "Level of detail" here is the general knob for how much compute and state a piece of the world gets, wherever that piece lives — building interiors and orgs (see Org cohorts, above) are two further subsystems this principle extends to, each getting the same hot/cold materialize treatment pawns get. Property remains the one deliberate exception: it stays flat data-only regardless of hotness, since a passive `propertyType` + ownership edge is already as cheap as data gets.
...
**Feature flags — the mechanism.** An Experience selecting "a subset of the game's systems" (above) is implemented as **feature flags**: every feature is gated by a named flag, and an Experience is a named set of flag values (plus its scenario, player role, and tuning overrides). This pushes the granularity below whole systems down to individual features — so an Experience can keep squads and combat but switch off economy, organizations, and trade ("command only squads, with a limited feature set"), or rebase the whole game into a different setting. It is also the **modding** hook (see the deferred "Experiences authored outside this document," above): a mod is a new flag-set plus tuning and content. The **default Experience turns every flag on** — that is the full game (no Experience = all flags on).

**Verified by contrast — two Experiences.** The flag mechanism is only testable if at least **two Experiences differ observably**: with only the full game, nothing proves the flags gate anything. So a **second, minimal Experience** ships alongside the default expressly as the verification fixture — it switches a clearly-observable set of features off (e.g. a squads-only slice: economy, organizations, and trade off; squads, combat, movement on). Verification is then cheap: a handful of **canary assertions** — under the minimal Experience the switched-off features are inert (no economy activity, no org-AI decisions, no caravans) while the kept ones work, and the default Experience behaves exactly like no-Experience — plus a static check that every flag is set by some Experience and referenced by some feature. A few observable canaries per flag-group prove the wiring without exhaustively testing every feature. Two Experiences are the minimum that makes the system verifiable; further Experiences are content, not additional test surface.
...
### Map legibility — biome patterns, water, and fields
The terrain layer is drawn to a deliberately high, at-a-glance-legible standard, and is **stylized rather than realistic**: biome kinds are told apart by **fine cartographic linework — thin lines and contours**, not colour or naturalistic texture. Each biome carries its own thin-line motif — contour isolines/hachure for mountains, sparse thin tree-glyphs or fine cross-hatching for forest, thin furrow lines for plains and fields, fine dotting for desert — for the look of a hand-drawn staff map in the antique dark style (Theme A), re-tinting cleanly per visual theme (see "Visual themes," above). **Forest legibility is functional, not just decorative:** a forest zone must be unmistakable, because a lumber camp can only be built in or near one (see "Terrain zones," above) — the player has to see where wood can come from.

Three water/field elements complete the picture:
- **Rivers** — the river paths already traced along zone boundaries (see "Terrain zones," above) are drawn as visible watercourses across the map, not left as data.
- **Lakes** — local closed water bodies, distinct from the global sea-level ocean, sitting in low basins so the map isn't dry everywhere the ocean doesn't reach.
- **Field zones** — a farm enterprise projects a cultivated **field zone** into the land around it (furrowed cropland) rather than being only a point icon, tying the economy visibly to the map the way a forest zone already backs a lumber camp.

What's fixed here is the *legibility standard and the stylized thin-line/contour direction* — biomes distinct by fine linework, forest unmistakable, rivers/lakes/fields present and readable — not the exact motifs, which are a visual-quality pass iterated in the prototype.
...
Tiers cross-fade smoothly into each other rather than popping. L2 geometry is procedurally generated per settlement, seeded consistently, and cached, using the **PCG city generator** — a text-driven mini-PCG runtime that faithfully reproduces Epic's PCGPrimitives city example (see `docs/pcg/city_generator.md`): city outline → arteries → districts → blocks → roads → lots → extruded buildings, all projected onto the sphere at the settlement's site. It is **scaled per settlement tier**: a megalopolis gets the full pipeline (arterial roads, districts, dense blocks); a city a lighter version; a village and a camp progressively simpler block-and-lot layouts with no arteries. The generated street network is the same one ambient residents walk (see "Pathfinding & Movement Spec" and the walking cats) and that in-settlement enterprises place against. L2 is the detailed scale that Scale-B combat and per-pawn weapon behaviors build on.
...
### Squad tray at a settlement
Squads sitting at a settlement (garrisoned, resting, or just passing through) would otherwise stack their markers on top of the settlement's own icon and bury it. Instead, every squad currently present at a settlement — within its marker footprint / capture zone — is laid out in a compact **squad tray**: a single row of squad glyphs docked just below the settlement marker, clear of the settlement icon. A squad that **enters** the settlement joins its tray and drops its own free-floating map marker; a squad that **leaves** resumes its normal marker. Each glyph in the tray keeps its faction colour and activity-status badge (above), and stays a fully clickable, selectable, orderable squad instance — clicking one selects it (with the universal selection frame — see UI conventions), and right-click orders / the hold-to-open options window act on it exactly as on any squad marker. All present squads appear regardless of team, so a contested settlement (see below) shows both sides' squads side by side in the tray, distinguished by faction colour. The tray belongs to the far/strategic zoom where squad markers live; at near/detail zoom squads resolve into individual pawns as usual (see the leader indicator, above), so the tray fades out with the markers. If more squads are present than fit one row, the tray wraps or caps to a readable width rather than sprawling.
...
**Tilt (pitch & yaw), resolved.** Separately from panning and rotation, the camera's viewing angle can be tilted off the default straight-down-onto-the-surface view, so the player can look across the world obliquely. It's driven by a **middle-mouse-button (wheel) drag**: dragging vertically changes **pitch** — from the default top-down (0°) down toward the horizon (clamped at roughly 70°, never past it) — and dragging horizontally changes **yaw**, orbiting that oblique view around the focused surface point so the same spot can be seen from different sides. A middle-button *click* with no drag stays the tooltip-pin action (see "Pinnable, nested tooltips," above); tilt and pin are told apart by the same movement threshold panning already uses — a drag past a few pixels is a tilt, a stationary press is a pin. Tilt is available at the world/strategic tiers and **persists** across panning and zooming, but **auto-flattens back toward top-down as the camera zooms fully out to the planet view**, where an oblique angle reads poorly against the globe's curvature. The building-interior sub-LOD keeps its own fixed top-down/orthographic camera (above) — tilt, like rotation, doesn't apply once you're inside a building.
...
**Snapshot — caravans & deficit coverage.** A trade block captures the caravan fleet (count, idle vs hauling split, and mean load factor as a throughput gauge), trade-house counts with total caravan capacity, and lifetime trade tallies — hauls completed, volume and profit moved, and the count of hauls that under-delivered because the buyer ran short of money. For each resource it records a **deficit-coverage landscape** that separates the three failure modes the raw deficit number conflates: a **production gap** (deficit beyond all surplus anywhere — no caravan can close it), a **logistics gap** (deficit that existing surplus *could* cover, set against what caravans actually delivered in the window, plus the median hop-distance from a deficit settlement to its nearest surplus), and an **affordability gap** (deficit settlements whose owner cannot pay for imports). Per-settlement rows also carry the units imported this window and a chronic-deficit streak, so a persistently underserved settlement is visible on its own row rather than only in the aggregate. A **spatial-reach** block answers how much of the world caravans actually touch — how many settlements ever host or receive a caravan versus are never touched at all, how concentrated the fleet sits at any moment (a presence index, as effective node count), and each trade-house's distinct reach — surfacing a "caravan desert" (a region of the map no caravan visits) as a number rather than something only spotted by eye; per-settlement rows carry the current caravan count, a cumulative visit tally, and the last tick a caravan was present.

**Event journal.** Recorded events span: org lifecycle — creation (with cause: worldgen seeding, squad-pairing, secession, founding a subordinate, founding an independent squad-org, or kind mutation) and destruction (with cause, and the disposition of treasury and property), kind and leadership-type changes, a perk taken, becoming political, becoming independent; political events — secession attempts and outcome, coups, succession crises (with successor type and legitimacy), leader deaths, war declared, peace made; economic events — a settlement captured (with method: direct, puppet, mutate, or replace), abandonment and repopulation, an enterprise founded (with cause) or idled, construction issued / completed / stalled, a squad recruited or disbanded (with cause), a caravan trade executed (source → destination, resource, quantity, buy/sell price, margin, hop-distance, profit, and the delivered fraction) or a caravan finding no profitable deal from its node, a bank loan taken or defaulted, a nationalization attempt and outcome; graph events — a slot created or broken, an alliance formed or broken, a covert edge exposed, an agent recruited, a sabotage or intel action run; and meta events — domination progress and the domination win.
