...
- **`military`** — a coordinating command, not a combat unit itself: its base **controlled-org/squad-hire capacity** (see "Organization capacity," above) is set much higher than an ordinary political org's, so a single `military`-kind org can hold many squad-kind orgs as control-slot occupants at once — a general staff sitting above an army of individually-simulated squads. Leader count follows the ordinary leadership-type table; it gets no override to 1, since it's administrative rather than a single-handler operation the way squad- and network-kind orgs are.
