...
- **Organization ≠ leadership type.** The org is the actor; **leadership type** (monarch / city council / entrepreneur / mercenary / economic / anarchic) is a **mutable attribute**. Org leaders make **decisions** — change leadership type, take a **perk/trait** (which may change type or leader count), set policy.
- **Leader count** derives from type/attributes: monarch 1, council ~9, mercenary 2, economic 1, anarchic 0 (no formal leader, headless) — **except `kind:'squad'` orgs, which always have exactly 1 leader regardless of leadership type.** This is a deliberate, resolved override: the leader-centric squad simulation's foundational claim that a squad has exactly one intelligent leader takes priority over the general leadership-type table for squad-kind orgs specifically. A squad that wants a second leadership seat gets there only through the **Council of Equals** perk (below), never through leadership-type alone.
...
**Org slot kinds — control / influence / covert.** Slots carry a `kind: 'control' | 'influence' | 'covert'`, layered onto the existing slot shape.
- **`control`** — the default kind: the occupant literally runs/commands that function (e.g. a garrison squad-org). Subordination is visible to everyone: inspecting another org shows its non-covert slots and their occupants read-only; inspecting your own org's control slots lets you create/fill one. Creating one uses a simple default-then-edit flow — it's filled immediately with a reasonable default occupant/salary/type, then edited or broken as needed; there's no separate occupant-picker dialog.
- **`influence`** — a softer effect: an occupant can sway decisions/policy without literal command authority. Not gated by any perk/skill system in v1 (none is designed yet). Same visibility rule as `control`.
- **`covert`** — a hidden, clandestine slot, invisible to everyone but the owning org. Filling one will eventually require a skill/perk check; until a perk system is designed, covert slots are locked/uncreatable — the field exists in the data model from day one so a later perk system won't need a structural change.
...
**Organizations relate through a graph, not a single link.** Beyond the one parent/child hierarchy line (territorial and political containment) and beyond a single slot, any two organizations can be connected by any number of simultaneous slot-relationships, each independently created, salaried, and broken — this is the organization graph: nodes are organizations, edges are slots. An organization commonly sits inside several such edges at once (e.g. independent, but paying salary into two allied organizations' control slots, while itself hosting a rival's covert-influence slot it doesn't know about).
...
**Covert influence, resolved.** Rather than staying permanently locked pending a future perk system, covert slots are unlocked by paying an upfront secrecy cost proportional to the target's own counter-intelligence (its leader's traits, garrison presence, or a future dedicated counter-intel stat) instead of requiring a perk. This keeps the "hidden until exposed" mechanic (an exposed covert slot behaves like the existing secret-ownership exposure above — a relations hit, and a chance of revolt) while giving the player a concrete, immediate way to attempt one rather than a feature that's simply off. Exposure risk should scale with how long the slot has existed and how large its effect is — a small covert nudge should be safer to maintain than an aggressive one.
...
- **Open question:** none of these actions cover an org (or a wealthy character) **founding a brand-new independent org from nothing** at runtime, as opposed to being seeded at worldgen or created through the player's own session start. Is spontaneous org founding in scope (e.g. an ambitious business splitting off a second enterprise, or a wealthy commoner founding a new mercenary company), or is the org population meant to be a closed set that only grows via `Expand`/recruit and only shrinks via dissolution or secession? See Org lifecycle, below — a real scope trade-off between emergent-org churn and a simpler, bounded org population that's easier to keep inside the entity budget (LOD section, below).

**Organization capacity — direct-control limits.** Rather than one flat cap on how deep or wide the org tree can grow, every organization has a small set of **capacity limits** on what it can directly hold at once — separate counters for **how many other organizations it can directly control** (control-kind slots it holds as controller — hired squads, subordinate businesses, puppets), **how many enterprises it can directly own**, **how many squads it can have on active hire** (a control slot specifically over a squad-kind org), **how many caravans it can run** (squads it has toggled into Caravan mode), **how much property it can directly own** (settlements, enterprises, vehicles combined), and **how many leader seats it has** (the leadership-type table above, or 1 for squad-kind orgs, as the base). Each capacity has a **base value set by org kind and leadership type** (a political capital naturally supports more subordinate slots than a small mercenary company; a business org's enterprise-ownership cap is naturally larger than a squad-kind org's) — the exact base numbers are a tuning pass, not fixed here.

**Perks are the primary way to raise a capacity above its base**, on top of whatever kind/leadership-type already grants — this is the resolution to "how big can an org get": not a hard global ceiling, but a chain of deliberate, permanent, trade-off-bearing choices (below), so a heavily-perked organization visibly earned its size rather than growing it for free. A capacity can also be *lowered* by a perk, as a genuine trade-off for a bigger benefit elsewhere (e.g. a perk that grants a strong economic bonus in exchange for a smaller squad-hire cap). This reframes the earlier open question about tree depth/width: **per-node width is capped (base + perks), tree depth is not** — a deep tree is still possible, it just requires many separate orgs each independently earning their own width capacity, which naturally self-limits without needing an arbitrary depth number.
...
| Expansive Charter | political org | +2 controlled-org capacity (more control slots it can hold as controller) | +1 tax-rate-sensitivity to loyalty — subordinates resent tax increases more |
| Mercantile Network | business-kind org | +2 enterprise-ownership capacity | −1 squad-hire capacity — a sprawling business struggles to also field muscle |
| Free Company Charter | squad-kind org | +2 squad-hire capacity (more squad-kind orgs it can keep on active hire at once) | +upkeep cost per hired squad — a bigger free company is a more expensive one |
...
**"Fake" independence — secret ownership.** A **hidden** ownership link (separate from the visible parent link) lets a puppet org look openly independent, or openly subordinate to a visible parent, while actually answering to a **hidden owner**. The hidden owner can only issue **high-level decisions** through this link — declare war/peace, shift relations, optionally set policy/taxes — while the puppet still runs its **own squads and economy** day-to-day (espionage/proxy influence, not full remote control). The secret is **exposable**: a chance/intel event can reveal it, at which point the link becomes public (the puppet becomes openly subordinate to its former hidden owner), the owner takes a **relations hit** with everyone else, and the puppet's internal factions may trigger a revolt/break instead of accepting the new arrangement. Both **AI and player** can use this — an ambitious org can spend influence to secretly subordinate a weaker org and direct proxy wars through it, and a human-led org can do the same.

**Org kind axis — nature vs governance.** `org.kind` is an axis orthogonal to leadership type: `kind ∈ {squad, political, business}` (with `religious` added later, and criminal/media as future possibilities) describes an org's **nature**, while `leadershipType` describes its **governance profile** — the two vary independently (a squad-kind org can have any leadership type, etc.). **Loyalty/opinion stays keyed on leadership type, not kind** — the two systems must not be conflated. Kind can change at runtime, mirroring how leadership type can change.
...
**Hot-region trigger — resolved.** A region is "hot" (materialized to full individual simulation) whenever any of the following is currently true within a fixed world-space radius of it, decided world-wide by game-state signals — never simply by where the player's camera happens to be pointed: an active combat, a squad executing a player-issued order, or a scripted/world event in progress. One fixed radius applies uniformly to every signal type (a tunable constant, not per-signal-type tuning) — simplicity over precision, revisit only if playtesting shows a specific signal type clearly needs a different radius. A region stays hot for a few seconds (a tunable grace period, proposed default 5–10s) after its last qualifying signal ends before demoting back to a cohort, to avoid flicker at the boundary when a fight or order briefly pauses. This keeps events happening elsewhere in the world properly simulated even when the player isn't currently looking at them. The game may additionally render cosmetic individuals or a heatmap near the camera purely for visual flavor, without that affecting the underlying simulation or its hot/cold state.
...
- **Split:** select a subset of a squad's pawns to form a new squad at the same spot, taking those concrete pawns with their strength/traits preserved; both squads re-seat afterward. Default subset is the current selection, or half the squad if nothing's selected.
...
**Open question — bankruptcy.** Squad-upkeep desertion is the only insolvency mechanic decided so far. There's no decided behavior for an org's treasury itself going negative from other sinks (construction payments, recruit costs, arbitrage purchases, founding a trading post) — see "Open design questions" at the end of this document.
...
Enterprise map nodes carry a type-prefixed label (e.g. "Oil depot · Corburg") and a distinct icon per kind on a shared marker base (oil depot = barrel, lumber camp = axe, mine = pick, farm = furrows, forge = anvil, armory = crossed blades, factory = gear) rather than a generic, unlabeled glyph. An in-city enterprise is its own clickable building node, visible only at close zoom; an outside enterprise draws as a camp-style marker linked to the road network like any other camp.
...
- **Bankruptcy / negative-money handling.** The only insolvency mechanic decided anywhere is squad-upkeep desertion (gradual pawn loss, reversible by paying again). Every other money sink — construction payments, recruit cost, founding a construction company or trading post — doesn't state whether an org's treasury can go negative at all, and if so, what the consequence is (a distressed-org state, forced asset liquidation, cascading desertion across all of an org's squads, dissolution).
...
This is the complete scoring model — no terrain, supply, morale, or leader-personality term feeds it. (Leader-trait weighting is a separate, only partly reconciled mechanism — see the archetype note under Sides & identity.)
...
### Sides as leaders (archetypes)
Each side is led by a leader — an identity attached to the side, not a map entity. Four archetypes: **Monarch, City Council, Entrepreneur, Mercenary**. Each archetype is a profile of utility-AI factor weights plus economy modifiers: e.g. Monarch defends and taxes steadily; Council grows population; Entrepreneur maximizes money and expansion; Mercenary attacks and leans on conquest. Two sides with different archetypes should visibly play differently under AI control.

A fifth archetype, **Warlord**, exists but is earned only through bandit conquest (see Bandits & outlaw factions) and is deliberately excluded from the normal random leader pool.

**Note:** there's an unresolved tension between this archetype model and a more detailed mechanism elsewhere in the org-leadership design, which scores an org's decisions by its individual leader character's personal traits (Just, Generous, Brave, Patient, Content, Cruel, Greedy, Wrathful, Cunning, Ambitious) rather than by one of four fixed profiles. It isn't decided whether: (a) archetype is a coarse preset that seeds a starting trait bundle later refined per-character; (b) the trait model supersedes the archetype model outright, and archetype becomes flavor/UI only; or (c) both apply independently and can conflict — e.g. an Entrepreneur-archetype side led by a passive-trait character. The stronger evidence (war-declare/seek-peace decisions keying off leader traits like "ambitious" and "content") leans toward the trait model being primary, but this needs a design ruling.
...
**Bandits never hold territory as bandits.** The moment a bandit faction captures a settlement, it stops being "bandits" — it converts into a legitimate, independent **Warlord**-led political org that holds territory, earns income, and can be diplomatically or militarily engaged like any other side. A fresh, landless bandit pool spawns afterward, so the poverty → bandits → conquest → new regime loop can repeat. Warlord is a leader archetype earned only this way — it's excluded from the normal random leader pool and from AI self-reform, so it only ever appears through conquest.
...
|---|---|---|---|
| **Settlement** | click a settlement marker | Settlement detail view | name, property type (camp/village/city/megalopolis), owner org (clickable, + its parent org clickable, or "independent"/"unowned"), territory area, money income (base + territory bonus breakdown), active resources with net production/step, enterprise count (running vs idle split) |
| **Squad** | click a squad marker | Squad detail view | id, faction, strength, control mode (auto/manual toggle), lock-auto toggle, order status (has order / idle), garrison target (if garrisoning), owning org (clickable "Belongs to: X" / "Independent") |
...
| **Organization** | click an org name anywhere (name in a panel, or the standalone Orgs list) | Org node view | name (drill-in link), leader name (+ heir picker if player-controlled), member count, ambition, tax rate (editable if player-controlled), leadership type + kind (editable if player-controlled), owned property (clickable settlement links), controlled slots (occupant + salary, editable if player-controlled), supply policy (per owned settlement, editable if player-controlled), income sources (editable if player-controlled), labor pool (player-controlled only), subordinate orgs (recursive subtree), ancestor breadcrumb upward to root |
| **Character** | click a character name (leader/heir/member link) | Character detail view | name, age/age-bucket, traits, residence settlement, org membership (clickable), org led (if a leader, clickable) + CK-style title, heir (if any, clickable) |
...
Not yet designed as inspectable (known gaps): enterprise as a standalone object (listed inside the settlement panel only, no dedicated enterprise inspector yet); vehicle as a standalone object outside squad loadout.
